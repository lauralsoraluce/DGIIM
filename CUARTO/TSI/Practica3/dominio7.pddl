...
(:action Navegar
    ...
    :precondition 
        (and 
            ...
            ( camino ?x ?y )
            ;la unidad tiene que estar ya reclutada (existir ya)
            ( not (sinReclutar ?u))
...
(:action Reclutar
    ...
    :precondition (and 
            ...
            ( sinReclutar ?u )
            ;tiene que estar construido el edificio
            ( construido ?e )
...
;la unidad u se puede mover de una localización a otra, aunque no estén directamente conectadas, siempre que haya un teletransportador en cada localización
(:action Teletransportar
    :parameters (?u - Unidad ?l1 - Localizacion ?l2 - Localizacion)
    :precondition 
    (and 
        ( en ?u ?l1 )
        ( not ( en ?u ?l2 ) )
        ...
        ;Existen dos edificios construidos (ambos teletransportadores), en las localizaciones dadas 
        ( exists ( ?edif - Edificio ?edif2 - Edificio ) 
            (and
                ( en ?edif ?l1 )
                ( construido ?edif )
                ( compTipoEdificio ?edif Teletransportador )
                ( en ?edif2 ?l2 )
                ( construido ?edif2 )
                ( compTipoEdificio ?edif2 Teletransportador )
            )
        )
    )
    :effect 
