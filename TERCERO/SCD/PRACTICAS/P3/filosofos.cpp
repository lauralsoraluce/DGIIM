...
void funcion_filosofos( int id )
{
  ...
  while ( true )
  {
    ...
    MPI_Recv(&peticion, 1, MPI_INT, id_camarero, MPI_ANY_TAG, MPI_COMM_WORLD, &estado);
	...
	MPI_Ssend(&peticion, 1, MPI_INT, id_ten_izq, etiq_liberar, MPI_COMM_WORLD);

	cout<< "Filósofo " <<id <<" suelta ten. der. " <<id_ten_der <<endl;
	// ... soltar el tenedor derecho (completar)
	MPI_Ssend(&peticion, 1, MPI_INT, id_ten_der, etiq_liberar, MPI_COMM_WORLD);
...
{
  int contador=0, etiq_emisor_aceptable;
  int valor; // etiqueta;
  MPI_Status estado ;       // metadatos de las dos recepciones
  ...
  while (true) {
	  ...
	  if (contador==4){
	  	etiq_emisor_aceptable = etiq_levantar;
	  }
	  else
