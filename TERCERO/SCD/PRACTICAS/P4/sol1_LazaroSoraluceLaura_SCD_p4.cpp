...
//
// NOMBRE: LAURA LÁZARO SORALUCE
// DNI: 45350527R
// CORREO: lazarosoraluce@correo.ugr.es
//
...
int main( int argc, char *argv[] )
{
   ...
   time_point<steady_clock> real_sec;
   ...
   while( true ) // ciclo principal
   {
      ...
      for( int i = 1 ; i <= 4 ; i++ ) // ciclo secundario (4 iteraciones)
      {
         ...
         real_sec = steady_clock::now();
         
         cout << "El retardo es de " << milliseconds_f(real_sec-ini_sec).count() << " milisegundos " << endl;
         
         if (milliseconds_f(real_sec-ini_sec).count()>10) {
         	exit(-1);
         }
      }
