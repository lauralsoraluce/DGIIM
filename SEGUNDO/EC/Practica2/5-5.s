...
# Si quisiesemos trabajar con datos de 64 bits, tendríamos que comentar el texto anterior y descomentar el siguiente que usa todas las funciones adaptadas para trabajar con 64 bits:
# call trabajarq 		# subrutina de usuario
# call imprim_Cq		# printf() de libC
# call acabar_Cq		# exit() de libC
# ret
...
# Hacemos una versión q de todas las funciones, utilizando los registros completos para poder trabajar con datos de 64 bits. 
trabajarq:
	mov $lista, %rbx 	# dirección del array lista
	mov longlista, %rcx 	# número de elementos a sumar
	call .mediaq		
	mov %rax, media	# Establecemos el valor de la media
...
bucleq:
	cmp %rdi, %rcx 	# comprobamos si el núm de sumas hechas es igual a de sumas total
	jz .Lterminar		# si son iguales, se han hecho todas las sumas y no hay que seguir haciendo más
	mov (%rbx, %rdi, 4), %r8 # copiamos el i-ésimo elemento si quedan sumas por hacer
	mov %r8, %r9 # cltd     
	sar $31, %r9  # cltd     # extendemos r8d a r9d:r8d con el bit más significativo de r8d
	add %r8, %rax 	# hacemos la suma (acumulador) en el registro eax
	adc %r9, %rdx		# suma el contenido del registro que cuenta los acarreos más el acarreo de la suma anterior y lo añade al contador de acarreos. 
	inc %rdi		# incrementamos el contador de sumas: i++
	jmp bucle		# volvemos a empezar el bucle
...
acabar_Cq: 			# void exit(int status);
 	mov media, %rdi 	# status: código a retornar (la suma)
 	call _exit 		# == exit(resultado)
 	ret 
