...
tree<T>::preorder_iterator::operator++() {
    ...
    if(!current.left_child().is_null()) {
        ...
        if (current.parent().is_null())
            current.remove();
        else if(!current.parent().right_sibling().is_null()) {
...
template<typename T>
typename tree<T>::preorder_iterator tree<T>::begin_preorder() {
    tree<T>::preorder_iterator it(root);
    return it;
}
...
tree<T>::const_preorder_iterator::const_preorder_iterator() {
    tree<T>::const_preorder_iterator it;
    it.level = 0;
    it.current.remove();
}
...
tree<T>::cbegin_preorder() const {
    tree<T>::const_preorder_iterator it(root);
    return it;
}
