...
Dictionary::iterator::iterator(const iterator &it) {
    iter = it.iter;
    curr_word = it.curr_word;
}
...
Dictionary::iterator &Dictionary::iterator::operator++() {
    ...
    while((*iter).valid_word) {
        ++iter;
        if (iter.get_level()>starter)
            curr_word.push_back((*iter).character);
        else if(iter.get_level()<starter)
            curr_word.pop_back();
        starter = iter.get_level();
    }
...
Dictionary::iterator Dictionary::begin() const {
    ...
    it.curr_word = (*words.get_root()).character;
}
