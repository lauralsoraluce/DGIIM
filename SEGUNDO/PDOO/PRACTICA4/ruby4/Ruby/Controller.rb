...
class Controller
  ...
  def self.HANGAR
    ...
    def combat() 
        result = @model.combat()
        case result
        when Deepspace::CombatResult::ENEMYWINS
          @view.lostCombatMessage()
        when Deepspace::CombatResult::STATIONESCAPES
          @view.escapeMessage()
        when Deepspace::CombatResult::STATIONWINS
          @view.wonCombatMessage()
          if @model.haveAWinner()
            @view.wonGameMessage()
            exit(0)
          end
        when Deepspace::CombatResult::NOCOMBAT
