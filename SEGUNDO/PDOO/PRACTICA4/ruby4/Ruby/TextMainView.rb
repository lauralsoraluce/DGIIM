...
class TextMainView
  ...
  def processCommand(command) 
    case command
      when Command::EXIT 
        Controller::Controller.instance.finish(0)
      ...
      when Command::DISCARDHANGAR 
        Controller::Controller.instance.discardHangar()
      ...
      when Command::COMBAT 
        puts "Combatiendo"
        Controller::Controller.instance.combat()
      when Command::NEXTTURN 
        Controller::Controller.instance.nextTurn()
    end
  end
  ...
  def updateView()
    @gameUI = Controller::Controller.instance.getUIversion()
    @state = Controller::Controller.instance.getState()
  end
  ...
  def showView() 
    while true # Hasta que se elija en el menú  Salir
      ...
      case @state
        when Deepspace::GameState::INIT
          command = getCommandInit()
        when Deepspace::GameState::BEFORECOMBAT 
          command = getCommandBeforeCombat()
        when Deepspace::GameState::AFTERCOMBAT 
      ...
      begin    # Choice and mount weapons or shields until go back
        ...
        when Element::WEAPON
          if operation == Operation::MOUNT
            Controller::Controller.instance.mount(elements,noElements)
          else 
            Controller::Controller.instance.discard(Controller::Controller.HANGAR,elements,noElements)
          end
        when Element::SHIELD
          if operation == Operation::MOUNT
            Controller::Controller.instance.mount(noElements,elements)
          else 
            Controller::Controller.instance.discard(Controller::Controller.HANGAR,noElements,elements)
          end
        end
        updateView()
  ...
  def discardMountedElements(element) 
      ...
      begin   # Choice and discard weapons or shields until go back
        ...
        case element
        when Element::WEAPON 
          Controller::Controller.instance.discard(Controller::Controller.WEAPON, elements, noElements)
        when Element::SHIELD 
          Controller::Controller.instance.discard(Controller::Controller.SHIELD, noElements, elements)
        end
        updateView()
    end while(option != Command::GOBACK.menu)
  end
